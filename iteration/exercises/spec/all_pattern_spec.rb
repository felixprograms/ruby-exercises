RSpec.describe 'all pattern' do
  ...
  it 'test 6' do
    ...
    all_busy = true
    # Your Code Here
    friend_status.each do |friend, state|
      all_busy = false if state != :busy
    end
    expect(all_busy).to be true
  end
  ...
  it 'test 7' do
    ...
    zip_codes.each do |zipcode|
      all_five_digits = false unless zipcode < 100000 && zipcode > 9999
    end      
  ...
  it 'test 8' do
    ...
    zip_codes.each do |name, code|
      all_five_digits = false unless code < 100000 && code > 9999
    end
